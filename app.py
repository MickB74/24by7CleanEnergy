...
# Analysis Results
    ...
    # Custom HTML Table with Tooltip
    html_table = """
    <style>
        .summary-table {
            width: 100%;
    ...
    for metric, value in zip(summary_data["Metric"], summary_data["Value"]):
        if "MW Match Productivity" in metric:
            tooltip_text = "Calculates the matched MWh per installed MW, allowing comparison of how efficiently different portfolios convert nameplate capacity into useful, hourly-matched clean energy."
            # Using a CSS-based tooltip for better compatibility than title attribute alone
            metric_html = f'<span class="tooltip-icon">{metric} ℹ️<span class="tooltiptext">{tooltip_text}</span></span>'
        else:
            metric_html = metric
